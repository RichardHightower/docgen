...
6. `Plain English Title`: Create a plain English title based on  "checkMap (com.cloudurable.docgen.mermaid.validation.classes.NoMapRule)".
...
private RuleResult checkMap(String side, String input, String line, int lineNumber) {
    ...
    if (matcher.find()) {
        return RuleResult.builder().ruleName("NoMap" + side).lineNumber(lineNumber).violatedLine(line).description("No Maps allowed, remove relationship from diagram or use \"*\" " + "and describe key association in description of relationship").build();
    }
