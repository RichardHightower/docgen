...
# Fix for 3 parseFile (com.cloudurable.docgen.parser.util.ClassVisitorParser)
...
Regenerate a mermaid sequence diagram based on the above guidelines titled parseFile (com.cloudurable.docgen.parser.util.ClassVisitorParser) using the Java code below by
...
8. `Plain English Title`: Create a plain English title based on  "parseFile (com.cloudurable.docgen.parser.util.ClassVisitorParser)".
...
[
    {
        "lineNumber": 8
,
        "violatedLine": "    ClassVisitorParser->>ClassVisitorParser: run(file)"
,
...
,
    ...
    },
    {
        "lineNumber": 10
,
        "violatedLine": "        ClassVisitorParser->>Exception: printStackTrace()"
,
        "ruleName": "No Method Calls In Descriptions Rule"
,
        "description": "No method calls in descriptions. instead of ```StringBuilder-->>Person: Return toString() result``` use ```StringBuilder-->>Person: Return toString result``` \ninstead of NO ```EmployeeService->>EmployeeRepo: updateEmployeeManager()``` use  YES ```EmployeeService->>EmployeeRepo: update Employee's Manager``` \n"
