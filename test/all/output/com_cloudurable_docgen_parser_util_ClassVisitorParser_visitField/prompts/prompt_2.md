...
# Fix for 3 visitField (com.cloudurable.docgen.parser.util.ClassVisitorParser)
...
Regenerate a mermaid sequence diagram based on the above guidelines titled visitField (com.cloudurable.docgen.parser.util.ClassVisitorParser) using the Java code below by
...
8. `Plain English Title`: Create a plain English title based on  "visitField (com.cloudurable.docgen.parser.util.ClassVisitorParser)".
...
[
    {
        "lineNumber": 1
,
        "violatedLine": "ClassVisitorParser->>VoidVisitorAdapter: visitField(parent, field)"
,
        "ruleName": "No Method Calls In Descriptions Rule"
,
        "description": "No method calls in descriptions. instead of ```StringBuilder-->>Person: Return toString() result``` use ```StringBuilder-->>Person: Return toString result``` \ninstead of NO ```EmployeeService->>EmployeeRepo: updateEmployeeManager()``` use  YES ```EmployeeService->>EmployeeRepo: update Employee's Manager``` \n"

    },
    {
        "lineNumber": 2
,
        "violatedLine": "VoidVisitorAdapter->>extractJavaDoc: extractJavaDoc(field.toString())"
,
        "ruleName": "No Method Calls In Descriptions Rule"
,
        "description": "No method calls in descriptions. instead of ```StringBuilder-->>Person: Return toString() result``` use ```StringBuilder-->>Person: Return toString result``` \ninstead of NO ```EmployeeService->>EmployeeRepo: updateEmployeeManager()``` use  YES ```EmployeeService->>EmployeeRepo: update Employee's Manager``` \n"
...
,
        "violatedLine": ""
,
        "ruleName": "Require Title"
,
        "description": "Must have a title, e.g., \n```---\ntitle: Generating toString\n---\nsequenceDiagram\n```\n"
