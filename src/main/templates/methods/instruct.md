...
* Diagrams must clearly represent important concepts. Use business domain-specific language when applicable.
...
* Exclude participant aliases in diagrams. No `as` keyword. Use original class/object names from code.
...
* Participants are always uppercase camel case.
...
* No orphan Participants: if the participant is not mentioned in the interactions then don't declare it in the participants section.
* Replace Throwable/Exception with "Handled error", "Reported error".
* Do not use `System.out` as a participant use `Console` instead 
