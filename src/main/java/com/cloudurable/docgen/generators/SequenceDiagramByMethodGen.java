...
import com.cloudurable.docgen.mermaid.validation.RuleRunner;
...
public class SequenceDiagramByMethodGen {
    ...
    private static RuleRunner buildRuleRunner(int count) {
        ...
        final var contentRule = List.of(new TitleRequiredRule(), new MismatchedTagsRule(), new RequiredTagsRule(), MermaidUtils.createRule());

        ruleRunner = RuleRunner.builder().contentRules(contentRule)
                .rules(rules).build();
        return ruleRunner;
    }
    ...
    public String generateSequenceFromMethod(String javaMethodSource, String methodName, String className, Consumer<Message> messageConsumer,
        ...
        context.forEach(messageConsumer);
        messageConsumer.accept(instructionMessage);
        ...
        return runValidationFeedbackLoop(javaMethodSource, title, instruction, request, messageConsumer,
    ...
    private String runValidationFeedbackLoop(String javaMethodSource, String title, String instruction,
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (chatResponse.getException().isPresent()) {
                ...
                messageConsumer.accept(chatChoice.getMessage());
                final var original = chatChoice.getMessage().getContent();
                responseConsumer.accept("----\n# ORIGINAL RESPONSE \n" + original);
                final var mermaidDiagram = extractSequenceDiagram(original);
                return validateMermaid(javaMethodSource, mermaidDiagram, title, 3, messageConsumer,
                        promptConsumer, responseConsumer);
            }
        }
        return "";
    ...
    private String validateMermaid(String javaMethodSource, final String originalMermaidDiagram, String title,
                                   int count, Consumer<Message> messageConsumer, Consumer<String> promptConsumer,
                                   Consumer<String> responseConsumer) {
        ...
        final var ruleRunner = buildRuleRunner(count);
        ...
        if (!checks.isEmpty()) {
            ...
            messageConsumer.accept(fixMessage);
            ...
            if (fixMermaidResponse.getResponse().isPresent()) {
                ...
                final var mermaidDiagram = extractSequenceDiagram(original);
                return validateMermaid(javaMethodSource, mermaidDiagram, title, count - 1, messageConsumer,
                        promptConsumer, responseConsumer);
            } else {
                return originalMermaidDiagram;
            }
